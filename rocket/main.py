...
def backtest(
    ...
    meta_filter: bool = typer.Option(
        ...
        help="Run ML meta-filter (EV on 0.32–0.85 bulk, max 3/day) and comparative report",
    ),
    min_prob: float = typer.Option(0.32, "--min-prob", help="Continuous-bulk floor (artifact spikes >0.85 discarded)"),
...
def compare_timeframes(
    ...
    limit: int = typer.Option(200, "--limit"),
    output: Optional[Path] = typer.Option(None, "--output", help="HTML tear sheet path"),
    min_prob: float = typer.Option(0.32, "--min-prob"),
