...
async def receive_chartink_webhook(request: Request):
    ...
    Supports GET/POST/PUT with the same payload rules as the bullish/bearish webhooks.
...
async def refresh_hourly_prices(db: Session = Depends(get_db)):
    ...
    try:
        ...
        def _skip_live_exit_if_broker_flat(record, fallback_sell_price: float) -> bool:
            """
            Guard for all exit types: if broker already shows exit/flat, do not send another SELL.
            """
            try:
                if live_trading.reconcile_intraday_exit_from_broker(
                    db, record, default_exit_reason="manual"
                ):
                    logger.info(
                ...
                logger.debug("Exit guard reconcile failed for %s: %s", record.stock_name, _rx)

            ikey = (record.instrument_key or "").strip()
            if not ikey:
                return False
                        ...
                        if quote_data and quote_data.get('last_price'):
                            ...
                            if not record.exit_reason:
                                ...
                                if exit_conditions['stop_loss']:
                                    if _skip_live_exit_if_broker_flat(record, new_option_ltp):
                                        exit_applied = True
                                    else:
                                    ...
                                    # Manual exit at broker: sync DB first; never send a second SELL if already flat
                                    if live_trading.reconcile_intraday_exit_from_broker(
                                        db, record, default_exit_reason="manual"
                                    ):
                                        exit_applied = True
                                        logger.info(
                                            "✅ TIME EXIT: %s reconciled from broker (already sold) — no SELL order",
                                            record.stock_name,
                                        )
                                    else:
                                        ikey_chk = (record.instrument_key or live_instrument_key or "").strip()
                                        bro_net = (
                                            live_trading.get_broker_net_long_qty_for_instrument(ikey_chk)
                                            if ikey_chk
                                            else None
                                        )
...
async def dashboard_sector_movers():
    """
    Top 3 Nifty sector indices by intraday % vs open when available (gainers) and bottom 3 (losers).
...
def smart_futures_run_picker_now(
    warmup_oi: bool = Query(
        True,
        description="Ignored — Smart Futures picker removed in Phase 1 consolidation.",
    ),
):
    """Phase 1: Smart Futures picker removed (candle-budget consolidation)."""
    return JSONResponse(
        status_code=410,
        content={
            "success": False,
            "message": "Smart Futures picker removed in Phase 1 consolidation",
        },
    )
...
async def dashboard_sector_movers_detail(sector: str, mode: str = "gainers"):
    ...
    with highest intraday % vs open when available (mode=gainers) or lowest % (mode=losers).
    """
    try:
