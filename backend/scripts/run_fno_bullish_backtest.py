...
repo. The artifact is served by the ``/api/fno-bullish/data`` router.
"""
...
def _default_out_path() -> Path:
    ec2 = Path("/home/ubuntu/trademanthan/data/fno_bullish_backtest.json")
    if ec2.parent.is_dir():
        return ec2
    return REPO_ROOT / "backend" / "data" / "fno_bullish_backtest.json"
...
def main() -> int:
    ...
    log.info("Loaded %s scanner rows from %s", len(rows), csv_path)
    if not rows:
