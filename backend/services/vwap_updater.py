...
def _try_live_exit(position, reason: str, option_contract: str):
    """
    Attempt live exit; return (ok, order_id, exit_manually, broker_fill_price).
    ...
    broker_fill_price: average SELL fill from Upstox when ok; use for sell_price/PnL (not LTP).
    """
    ...
    if result.get("success") or result.get("skipped_duplicate"):
        afp = result.get("average_fill_price")
        try:
            fill_px = float(afp) if afp is not None and float(afp) > 0 else None
        except (TypeError, ValueError):
            fill_px = None
        return True, result.get("order_id"), False, fill_px
    return False, None, False, None


def _exit_sell_price(broker_fill_price, fallback_ltp: float, position) -> float:
    """Prefer broker SELL average; else last sell_price; else LTP fallback."""
    if broker_fill_price is not None and broker_fill_price > 0:
        return float(broker_fill_price)
    if getattr(position, "sell_price", None) and float(position.sell_price) > 0:
        return float(position.sell_price)
    if fallback_ltp and fallback_ltp > 0:
        return float(fallback_ltp)
    if getattr(position, "buy_price", None) and float(position.buy_price) > 0:
        return float(position.buy_price)
    return 0.0
...
def _skip_exit_if_already_closed_at_broker(
    ...
    logger.warning(
        ...
        "not marking sold from LTP; caller may attempt live exit",
        getattr(position, "stock_name", "?"),
        bro_net,
    )
    return False
        ...
        for idx, position in enumerate(open_positions, 1):
            try:
                            ...
                            if (option_type == 'CE' and new_stock_ltp < new_vwap) or \
                               (option_type == 'PE' and new_stock_ltp > new_vwap):
                                ...
                                if _skip_exit_if_already_closed_at_broker(db, position, now, new_option_ltp):
                                    continue
                                live_ok, live_order_id, exit_manual, fill_px = _try_live_exit(
                                    position, 'stock_vwap_cross', option_contract
                                )
                                if exit_manual:
                                    ...
                                    position.sell_price = _exit_sell_price(fill_px, new_option_ltp, position)
                                    if live_order_id:
                                    ...
                                    if position.buy_price and position.qty:
                                        ...
                                        position.pnl = 0.0
                                    logger.critical(
                                        f"✅ FORCED EXIT: {stock_name} on VWAP cross sell=₹{position.sell_price:.2f}, PnL=₹{position.pnl:.2f}"
                                    )
                                    updates_made.append(f"🚨 EXITED: stock_vwap_cross at ₹{position.sell_price:.2f}")
                                    updated_count += 1
                ...
                if new_option_ltp > 0:
                    ...
                    # Use new_option_ltp if > 0, otherwise fallback to position.sell_price if available
                    ...
                    if position.buy_price and position.qty:
                        if new_option_ltp > 0:
                            ...
                            # Fallback: Use existing sell_price if new_option_ltp is 0 or negative
                            old_pnl = position.pnl or 0.0
                            new_pnl = (position.sell_price - position.buy_price) * position.qty
                            position.pnl = new_pnl
                            flag_modified(position, 'pnl')
                            updates_made.append(f"P&L: ₹{old_pnl:.2f}→₹{new_pnl:.2f} (using sell_price)")
                            logger.info(f"📊 {stock_name}: PnL updated to ₹{new_pnl:.2f} (Buy: ₹{position.buy_price:.2f}, Sell: ₹{position.sell_price:.2f}, Qty: {position.qty}) - using sell_price fallback")
                            pnl_calculated = True
                    ...
                    if not pnl_calculated:
                        ...
                        if not position.qty:
                            missing.append("qty")
                        if new_option_ltp <= 0 and (not position.sell_price or position.sell_price <= 0):
                            missing.append("option_ltp or sell_price")
                        logger.warning(f"⚠️ {stock_name}: PnL NOT calculated - missing: {', '.join(missing) if missing else 'unknown reason'}")
                        logger.warning(f"   buy_price: {position.buy_price}, qty: {position.qty}, new_option_ltp: {new_option_ltp}, sell_price: {position.sell_price}")
                else:
                ...
                if position.exit_reason is None and position.status != 'sold':
                    ...
                    if exit_triggered and exit_reason_to_set:
                        if _skip_exit_if_already_closed_at_broker(db, position, now, new_option_ltp):
                            ...
                            continue
                        live_ok, live_order_id, exit_manual, fill_px = _try_live_exit(
                            position, exit_reason_to_set, option_contract
                        )
                        if exit_manual:
                            from sqlalchemy.orm.attributes import flag_modified
                            position.exit_reason = "Exit Manually"
                            ...
                            # CRITICAL: sell_price from broker SELL fill when available
                            from sqlalchemy.orm.attributes import flag_modified
                            position.sell_price = _exit_sell_price(
                                fill_px,
                                new_option_ltp if new_option_ltp > 0 else old_option_ltp,
                                position,
                            )
                            flag_modified(position, 'sell_price')
    ...
    try:
        ...
        for trade in stocks_to_process:
            try:
                ...
                # ====================================================================
                # RECALCULATE CANDLE SIZE FOR ALL STOCKS
                # ====================================================================
                # Recalculate candle size for ALL stocks in each cycle to ensure
                # candle size is always up-to-date with latest data. This is especially
                # important for stocks from 11:15 AM, 1:15 PM, and 2:15 PM alerts.
                ...
                # 4. OR it's a 10:15 AM alert at Cycle 1 (10:30 AM) - ALWAYS recalculate
                should_recalculate = False
                
                # CRITICAL: For 10:15 AM alerts at Cycle 1 (10:30 AM), ALWAYS recalculate candle size
                if is_10_15_alert and cycle_number == 1:
                    should_recalculate = True
                    logger.info(f"🔄 Cycle {cycle_number} - {stock_name}: 10:15 AM alert - Will recalculate candle size at 10:30 AM")
                
                if trade.instrument_key:
                    # Always recalculate if instrument_key exists, especially if:
                    # - Candle size was never calculated (ratio is None)
                    # - Status indicates retry needed (None, "Retry", "Pending", "Skipped")
                        ...
                        if option_candles:
                            ...
                            if current_day_candle and previous_day_candle:
                                ...
                                if previous_size > 0:
                                    ...
                                    # For 10:15 AM alerts at Cycle 1, still try to calculate even if previous is zero
                                    if is_10_15_alert and cycle_number == 1 and current_size > 0:
                                        # Use a default previous size (e.g., current_size / 2) to calculate ratio
                                        # This allows calculation even when previous day data is incomplete
                                        estimated_previous_size = current_size / 2.0
                                        candle_size_ratio = current_size / estimated_previous_size
                                        candle_size_passed = (candle_size_ratio < 7.5)
                                        trade.candle_size_ratio = candle_size_ratio
                                        trade.candle_size_status = "Pass" if candle_size_passed else "Fail"
                                ...
                                if is_10_15_alert and cycle_number == 1:
                                    # Set status to Retry so it will be attempted again
                                    trade.candle_size_status = "Retry"
                                    logger.warning(f"⚠️ Cycle {cycle_number} - {stock_name}: Missing candle data for 10:15 alert (current: {bool(current_day_candle)}, previous: {bool(previous_day_candle)}) - Will retry")
                                else:
                            ...
                            # API returned None - for 10:15 AM alerts at Cycle 1, set to Retry
                            if is_10_15_alert and cycle_number == 1:
                                trade.candle_size_status = "Retry"
                                logger.warning(f"⚠️ Cycle {cycle_number} - {stock_name}: get_option_daily_candles_current_and_previous returned None for 10:15 alert - Will retry")
                            else:
                            ...
                            if trade.exit_reason is None:
                                ...
                                if trade.stop_loss and current_option_ltp_for_pnl <= trade.stop_loss:
                                    ...
                                    if _skip_exit_if_already_closed_at_broker(db, trade, now, current_option_ltp_for_pnl):
                                        ...
                                        continue
                                    live_ok, live_order_id, exit_manual, fill_px = _try_live_exit(
                                        trade, 'stop_loss', trade.option_contract
                                    )
                                    if exit_manual:
                                        from sqlalchemy.orm.attributes import flag_modified
                                        ...
                                        trade.sell_time = now
                                        trade.status = 'sold'
                                        trade.sell_price = _exit_sell_price(
                                            fill_px, current_option_ltp_for_pnl, trade
                                        )
                                        if live_order_id:
                                            trade.sell_order_id = live_order_id
                                        if trade.buy_price and trade.qty:
                                            trade.pnl = (trade.sell_price - trade.buy_price) * trade.qty
                                        flag_modified(trade, 'exit_reason')
                                        ...
                                        flag_modified(trade, 'pnl')
                                        logger.warning(
                                            f"🛑 Cycle {cycle_number} - {stock_name}: STOP LOSS EXIT sell=₹{trade.sell_price:.2f}, PnL: ₹{trade.pnl:.2f}"
                                        )
                                            ...
                                            if (option_type == 'CE' and stock_ltp_exit < stock_vwap_exit) or \
                                               (option_type == 'PE' and stock_ltp_exit > stock_vwap_exit):
                                                ...
                                                if _skip_exit_if_already_closed_at_broker(db, trade, now, current_option_ltp_for_pnl):
                                                    ...
                                                    continue
                                                live_ok, live_order_id, exit_manual, fill_px = _try_live_exit(
                                                    trade, 'stock_vwap_cross', trade.option_contract
                                                )
                                                if exit_manual:
                                                    from sqlalchemy.orm.attributes import flag_modified
                                                    trade.exit_reason = "Exit Manually"
    ...
    try:
        ...
        for position in open_positions:
            try:
                ...
                # FINAL SAFETY CHECK: Re-query from database by ID to get absolute latest state
                # This prevents race conditions where exit_reason was set by another process between refresh() and here
                fresh_position = db.query(IntradayStockOption).filter(
                    IntradayStockOption.id == position.id
                ).first()
                
                if not fresh_position:
                    logger.warning(f"⚠️ FINAL CHECK: {stock_name} (ID: {position.id}) no longer exists in database - skipping")
                    continue
                
                if fresh_position.exit_reason is not None:
                    logger.warning(f"⚠️ FINAL CHECK: Skipping {stock_name} - exit_reason already set to '{fresh_position.exit_reason}' (NOT overwriting with time_based)")
                    continue
                if fresh_position.status == 'sold':
                    logger.warning(f"⚠️ FINAL CHECK: Skipping {stock_name} - status already 'sold' (NOT overwriting)")
                    continue
                ...
                if bro_net is not None and bro_net <= 0:
                    if live_trading.reconcile_intraday_exit_from_broker(
                        db, position, default_exit_reason="time_based"
                    ):
                        logger.info(
                            "⏰ EOD 3:25: %s reconciled closed from broker SELL fill (net qty=%s)",
                            stock_name,
                            bro_net,
                        )
                        closed_count += 1
                        continue
                    logger.warning(
                        "⏰ EOD 3:25: %s broker net long=%s but no SELL fill — will attempt live exit",
                        stock_name,
                        bro_net,
                    )
                ...
                exit_time_str = now.strftime('%Y-%m-%d %H:%M:%S IST')
                live_ok, live_order_id, exit_manual, fill_px = _try_live_exit(
                    position, 'time_based', option_contract
                )
                if exit_manual:
                    from sqlalchemy.orm.attributes import flag_modified
                    position.exit_reason = "Exit Manually"
                ...
                fp = float(option_ltp) if option_ltp and float(option_ltp) > 0 else 0.0
                position.sell_time = now
                position.exit_reason = 'time_based'
                position.status = 'sold'
                position.sell_price = _exit_sell_price(fill_px, fp, position)
                if live_order_id:
                    position.sell_order_id = live_order_id
                if position.buy_price and position.qty:
                    position.pnl = round(
                        (float(position.sell_price) - float(position.buy_price)) * int(position.qty),
                        2,
                    )
                    flag_modified(position, "pnl")
                flag_modified(position, "sell_price")
