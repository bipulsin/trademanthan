...
def load_nse_holiday_dates(from_date: date, to_date: date) -> set:
    """Holiday dates from DB calendar + NSE circular + Sambhav V1 holiday set."""
    holidays: set = set()
    try:
    ...
    try:
        ...
        holidays |= sambhav_holiday_dates(from_date, to_date)
    except Exception:
        logger.debug("sambhav holiday set unavailable", exc_info=True)
    return holidays
