"""Shadow-only READY dwell (10m) + entry distance guard.

Instrumentation only — does **not** change card visibility or Take Trade.
Live flip stays behind ``READY_DWELL_ENTRY_LIVE`` (default off). Collect one
full session in ``kavach_ready_consistency_log.inputs.dwell_entry_shadow``
(+ state table) before go-live sign-off.

Approved formula (Option A):
  min_gap_pts = max(0.3% * price, 300 / lot)
...
MIN_GAP_PCT = 0.003
MIN_INR_RISK_FLOOR = 300.0
READY_DWELL_MINUTES = 10
ENTRY_EMA5_WARN_PCT = 0.5  # same band as ENTRY_EMA5_TOL_PCT; WARN only in shadow
...
def min_gap_pts(price: Optional[float], lot: int) -> Optional[float]:
    """Per-symbol floor: max(0.3%·price, ₹300/lot)."""
    if price is None or price <= 0:
        return None
    lot_i = max(int(lot or 1), 1)
    return max(float(MIN_GAP_PCT) * float(price), float(MIN_INR_RISK_FLOOR) / lot_i)
...
def evaluate_entry_distance_guard(
    *,
    ...
    """Render-time distance / side / stack checks (shadow or future live)."""
    ref_price = entry if entry is not None else (ema5 if ema5 is not None else price)
    gap_floor = min_gap_pts(ref_price, lot)
    ...
    return {
        ...
        "min_inr_risk_floor": MIN_INR_RISK_FLOOR,
...
def build_dwell_entry_shadow(
    ...
    *,
    ...
    # Shadow entry = live 10m EMA5 (approved path), not lagging audit alone.
    ...
    dist = evaluate_entry_distance_guard(
        ...
        ema5=live_ema5,
        ema10=live_ema10,
        price=live_price if live_price is not None else shadow_entry,
        lot=lot,
    )
    ...
    payload: Dict[str, Any] = {
        ...
        "formula": "max(0.003*price, 300/lot)",
        ...
        # Explicit breakout for session report
