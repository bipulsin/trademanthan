    ...
    def start(self):
        ...
        if not self.is_running:
            ...
            try:
                ...
                print(f"✅ Master Stock Scheduler started - Jobs: {len(self.scheduler.get_jobs())}", flush=True)
            except Exception as e:
                logger.error(f"❌ Failed to start Master Stock Scheduler: {e}", exc_info=True)
                print(f"❌ Failed to start Master Stock Scheduler: {e}", flush=True)
                raise
