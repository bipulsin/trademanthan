...
## Server Information
- **Base URL**: `http://3.6.199.247:8000`
- **Endpoints**:
...
2. **URL**: Enter one of the following:
   - For Bullish alerts: `http://3.6.199.247:8000/scan/chartink-webhook-bullish`
   - For Bearish alerts: `http://3.6.199.247:8000/scan/chartink-webhook-bearish`
   - For Auto-detect: `http://3.6.199.247:8000/scan/chartink-webhook`
...
1. Check the response - should show:
   ...
   ```
2. Check backend logs: `ssh ubuntu@3.6.199.247 "tail -f /tmp/uvicorn.log"`
3. Check database: Query `intraday_stock_options` table for the stocks you sent
4. Check frontend: Visit `http://3.6.199.247:8000/scan.html` to see the stocks
...
```bash
ssh -i /path/to/TradeM.pem ubuntu@3.6.199.247 "tail -100 /tmp/uvicorn.log | grep -A 5 'Processing stock'"
```
...
```bash
ssh -i /path/to/TradeM.pem ubuntu@3.6.199.247 "cd /home/ubuntu/trademanthan && python3 -c \"
...
- Visit: `http://3.6.199.247:8000/scan.html`
...
curl http://3.6.199.247:8000/scan/upstox/status

# Check backend logs
ssh -i /path/to/TradeM.pem ubuntu@3.6.199.247 "tail -200 /tmp/uvicorn.log | grep -i 'error\|enrichment\|failed'"
```
...
```bash
curl -X POST "http://3.6.199.247:8000/scan/chartink-webhook-bullish" \
...
```bash
curl -X POST "http://3.6.199.247:8000/scan/chartink-webhook-bearish" \
  -H "Content-Type: application/json" \
  -d '{
    "stocks": "HDFCBANK",
    "trigger_prices": "1650.00",
