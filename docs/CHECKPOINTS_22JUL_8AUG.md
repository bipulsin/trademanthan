...
| 8 | Backend FSM vs Pine confidence | **OPEN — concrete gaps** | Same | Live TV script = `TWCTO_Kavach_v3_0`. Grade banding/stretch defaults **aligned**. Remaining tickets: **I8-A** readiness dir (`BULLISH_STATES` vs Pine `buyEligible`); **I8-B** checklist `_norm_grade` accepts `A!`/`B!` as READY, Pine `gradeReadyLevel` does not; **I8-C** DIR CONFLICT MACD uses histogram vs Pine Votes line-vs-signal. No live code change until tickets decided. |
