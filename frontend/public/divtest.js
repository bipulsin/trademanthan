...
(() => {
  ...
  const API_BASES = ['/api/divtest', '/divtest'];
  ...
  async function loadResults() {
    ...
    const data = await api(`/results?${q}`);
  ...
  function subscribeJob(jobId) {
    ...
    state.jobId = jobId;
    const es = new EventSource(`/api/divtest/jobs/${jobId}/events`);
    state.es = es;
  ...
  $('runForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const res = await api('/run-backtest', {
        method: 'POST',
