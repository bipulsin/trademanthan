...
(function () {
  ...
  async function loadHealth() {
    ...
    const tgPub = document.getElementById('tgPublicSignals');
    if (tgPub) {
      try {
        const s = await api('/api/tarang/telegram/settings');
        tgPub.checked = !!s.telegram_public_signals;
      } catch (e) {
        tgPub.checked = false;
      }
    }
  ...
  function renderBacktest(bt) {
    ...
    const wrHtml = omitPfDd
      ? '<div class="tg-metric"><span>Win rate / PF / expectancy</span><strong>Too few trades to be meaningful</strong></div>'
    ...
    metrics.innerHTML = `
      ...
      ${wrHtml}
      ${omitPfDd ? '' : `<div class="tg-metric"><span>Profit factor</span><strong>${st.profit_factor != null ? Number(st.profit_factor).toFixed(2) : '—'}</strong></div>`}
      ${omitPfDd ? '' : `<div class="tg-metric"><span>Expectancy</span><strong>${st.expectancy != null ? fmtInr(st.expectancy) : '—'}</strong></div>`}
  ...
  const btnScr = document.getElementById('btnScreen');
  if (btnScr) btnScr.addEventListener('click', async () => {
    const status = document.getElementById('statusLine');
